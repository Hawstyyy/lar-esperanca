...
try:
    query = """
    SELECT p.id_paciente, p.nome_paciente, s.temperatura, s.pressao_cardiaca, 
           CONCAT(s.pressao_sistolica, '/', s.pressao_diastolica) as pressao_arterial, 
           s.saturacao, p.status_paciente
    FROM paciente p
    JOIN sinal_vital s ON p.id_paciente = s.id_paciente
    ORDER BY p.id_paciente  -- Garantir a ordem
    """
    ...
    print(dic_teste)  # Para verificação dos pacientes
    ...
    for idx, (id_paciente, info) in enumerate(list(pacientes.items())[start_index:start_index + 10]):
        ...
        for idy, valor in enumerate([paciente_truncado] + info['dados']):
            ...
            if idy == 2:  # Add BPM no label
                ...
                if info['dados'][-1] == 0:  # 0 sendo OK e 1 sendo RISCO
        ...
        linhas.append((linha_frame, linha_labels, id_paciente))  # Adiciona o frame, as labels e o id à lista
    ...
    for idx, (linha_frame, linha_labels, id_paciente) in enumerate(linhas):
        if idx < 10:  # Limita até 10 linhas
            if (x * 10 + idx) < len(dic_teste):  # Verifica se existe paciente para essa linha
                ...
                # Atualiza status
                linha_labels[5].configure(text='', image=img('imagens/check_icon.png', 30, 30) if info['dados'][-1] == 0 else img('imagens/alert_icon.png', 30, 26))
            else:
